...
3. **If no Tech Lead implementation plan comment exists — stop immediately.** Do not proceed with the issue description or your own interpretation. Report back to the orchestrator:
   ...
   Wait for the plan to be created, posted as a Linear comment, and explicitly approved by the user before continuing.
...
Stage only the files you changed. Write a commit message that explains *why*, not just what:
...
Run a smoke test as appropriate for the project (see `CLAUDE.md` for the smoke test command). Report back to the user and wait for their confirmation before pushing.
